...
public class SouthFaceWriter extends FaceWriterWrapper {
	@Override
	public void colorizeAndBufferQuad(BlockRenderer blockRenderer, ModelBoundsData modelData, Block<?> block, int x, int y, int z, int color) {
		blockRenderer.colorizeQuad(block, x, y, z, Direction.SOUTH, 0, 0, 1, 1.0F - modelData.maxZ, 0, 1, 0, modelData.maxY, modelData.minY, -1, 0, 0, 1.0F - modelData.minX, 1.0F - modelData.maxX, color);
	...
	public void vertexOne(ModelBoundsData model, int color) {
		...
		man.setPos(model.x0, model.y1, model.z1);
		man.setUv(minU, maxV);
		man.setColor(color);
