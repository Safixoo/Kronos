...
import static dev.safixo.client.render.pipelines.terrain.meshing.model.light.data.ArrayLightDataCache.*;
...
class AoFaceData {
    ...
    public void initLightData(LightDataAccess cache, Vector3i pos, int direction, boolean offset) {
        final int x = pos.x;
        final int y = pos.y;
        final int z = pos.z;
        ...
        if (offset) {
            adjX = x + Direction.x(direction);
            adjY = y + Direction.y(direction);
            adjZ = z + Direction.z(direction);
        } else {
            adjX = x;
            adjY = y;
            adjZ = z;
        ...
        int[] faces = AoNeighborInfo.get(direction).faces;
    ...
    private static int calculateCornerBrightness(int a, int b, int c, int d, boolean aem, boolean bem, boolean cem, boolean dem) {
        ...
        if (aem) {
            a = MathExt.getLightmapCoord(15, 15);
        }
        if (bem) {
            b = MathExt.getLightmapCoord(15, 15);
        }
        if (cem) {
            c = MathExt.getLightmapCoord(15, 15);
        }
        if (dem) {
            d = MathExt.getLightmapCoord(15, 15);
        }
