...
public class SectionMesher {
	...
	public static boolean rebuild(SectionRender section, CameraData camera, SectionManager sectionManager, World world, Set<TileEntity> tileSet) {
		...
		boolean insideSection = cameraChunkX == sectionX && cameraChunkZ == sectionZ;
		...
		if (!airEmpty) {
			...
			if (insideSection) {
				for (int i = 0; i < 4096; i++) {
					SectionCache.VISITED_CENTER_BLOCKS[i] = PrimitivesFlags.SOLID_CULL_MASK[SectionCache.CENTER_BLOCKS[i] & 0xFF];
				}
			}
